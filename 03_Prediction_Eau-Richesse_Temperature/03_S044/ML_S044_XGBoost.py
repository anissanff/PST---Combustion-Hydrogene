...
# MODÈLE : XGBoost (meilleur que Ridge après tests)
# ============================================================
...
from sklearn.ensemble import GradientBoostingRegressor  # ← XGBoost sklearn
...
# -----------------------------
# 5) Pipeline XGBoost (remplace Ridge)
# -----------------------------
model = Pipeline(steps=[
    ("scaler", StandardScaler()),  # XGBoost aime scaler
    ("reg", GradientBoostingRegressor(
        n_estimators=100,      # 100 arbres
        learning_rate=0.1,     # vitesse apprentissage
        max_depth=3,           # profondeur arbres
        random_state=0         # reproductible
    ))
])

# -----------------------------
# 6) Cross-validation par groupes
# -----------------------------
...
print(f"GroupKFold XGBoost (n_splits={n_splits}, n_paliers={n_groups})")
...
# -----------------------------
# 7) Entraîner XGBoost final
# -----------------------------
model.fit(X, y)

# -----------------------------
# 8) Exemple prédiction
# -----------------------------
example = pd.DataFrame({"phi": [0.75], "steam_pct": [5.0]})
pred_T = model.predict(example)[0]
print(f"Prédiction XGBoost T_CH9 (φ=0.75, vapeur=5%) : {pred_T:.2f}")

# -----------------------------
# 9) Prédictions + erreurs
# -----------------------------
df["T_pred"] = model.predict(X)
df["erreur"] = df["T_CH9"] - df["T_pred"]
df["erreur_pct"] = (df["erreur"] / df["T_CH9"]) * 100
df.to_csv("dataset_with_predictions_xgboost.csv", index=False)
...
fig.suptitle(f'Modèle XGBoost: T_CH9 (R²={r2_test.mean():.3f}, MAE={mae_test.mean():.1f}°C)', 
...
ax1.set_xlabel('T mesurée (°C)'); ax1.set_ylabel('T prédite (°C)')
ax1.set_title(f'① Validation XGBoost (R²={r2_test.mean():.3f})', fontweight='bold')
...
ax2.set_xlabel('Erreur relative (%)'); ax2.set_ylabel('Fréquence')
ax2.set_title('② Distribution Erreurs', fontweight='bold')
ax2.legend(); ax2.grid(True, alpha=0.3, axis='y')

# G3: Performance par φ
...
x = np.arange(len(richesses)); width = 0.35
...
ax3.set_ylabel('Erreur (°C)'); ax3.set_xlabel('Richesse φ')
...
ax4.set_xlabel('Vapeur (%)'); ax4.set_ylabel('T_CH9 (°C)')
ax4.set_title('④ Effet Vapeur par φ', fontweight='bold')
ax4.legend(); ax4.grid(True, alpha=0.3)

plt.savefig('presentation_xgboost.png', dpi=300, bbox_inches='tight', facecolor='white')
plt.show()
print("✅ XGBoost PRÉSENTATION SAUVEGARDÉE : presentation_xgboost.png")
